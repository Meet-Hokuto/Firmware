...
__EXPORT int read_uart_term3_main(int argc, char *argv[]);


static bool thread_should_exit = false; /*Ddemon exit flag*/
static bool thread_running = false;  /*Daemon status flag*/
static int read_uart_term3_task;

/**
 * Main loop
 */
int read_uart_term3_thread_main(int argc, char *argv[]);
...
static void usage(const char *reason)
{
    ...
    fprintf(stderr, "usage: read_uart_term3_sensor {start|stop|status} [param]\n\n");
    exit(1);
}

int read_uart_term3_main(int argc, char *argv[])
{
    if (argc < 2) {
    ...
    if (!strcmp(argv[1], "start")) {
        ...
        read_uart_term3_task = px4_task_spawn_cmd("read_uart",
                         SCHED_DEFAULT,
                         SCHED_PRIORITY_MAX - 5,
                         2000,
                         read_uart_term3_thread_main,
...
int read_uart_term3_thread_main(int argc, char *argv[])
{
    ...
    /*初始化数据结构体 */
    struct read_uart_term3_s _uart_data;
    memset(&_uart_data, 0, sizeof(_uart_data));
    _uart_data.is_valid = false;
    /* 公告主题 */
    orb_advert_t read_uart_term3_pub = orb_advertise(ORB_ID(read_uart_term3), &_uart_data);
    orb_publish(ORB_ID(read_uart_term3), read_uart_term3_pub, &_uart_data);
    ...
    while(!thread_should_exit) {
	   ...
	   // _uart_data.datax = atoi(_uart_data.datastr);//将字符串转换成整形数据     ---室内跟踪
            _uart_data.datax =atoi(_uart_data.datastr);//将字符串转换成整形数据    ---室内避障雷达单位为mm
            //printf("[Read_uart]sensor.data=%d\n",_uart_data.datax);
            //orb_publish(ORB_ID(read_uart_term3), read_uart_pub, &_uart_data);
        }
...
/*	if (data == 'x'){
        ...
        if (data == 'Y'){
            ...
            strncpy(_uart_data.datastr,buffer,4);// 复制字符串Buffer中前４个数字到Datastr中
	   // _uart_data.datax = atoi(_uart_data.datastr);//将字符串转换成整形数据            ---室内跟踪
            _uart_data.datay =atoi(_uart_data.datastr);//将字符串转换成整形数据            ---室内避障雷达单位为mm
			}
